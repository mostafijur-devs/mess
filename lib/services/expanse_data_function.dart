...
class ExpanseDataFunction extends ExpanseRepository{
  ...
  Future<void> addExpanse(Expanse expanse) async {
    ...
    await db!.insert(expanseTableName, expanse.toJson());
     ...
     List<Map<String,dynamic>> memberData = await db.query(memberTableName, where:'id = ?', whereArgs: [expanse.memberId]);
    // await db.query(table)
    if(memberData.isNotEmpty){
      final currentAmount = memberData.first[columnPersonalAmount]  ?? 0;
      final newAmount = currentAmount + (expanse.amount ?? 0);

      await db.update(memberTableName,{memberTableName:newAmount},where: 'id = ?', whereArgs: [expanse.memberId]);
    }
  ...
  Future<void> deleteExpanse(int id) async {
    Database? db = await dbCreate.database;
    await db!.delete(expanseTableName,where: '$expanseColumnId = ?',whereArgs: [id]);
  }

  @override
  Future<List<Expanse>> getExpanses() async{
   Database? db = await dbCreate.database;
   List<Map<String,dynamic>> expanseMapList =await db!.query(expanseTableName);
   return List.generate(expanseMapList.length,  (index) => Expanse.fromJson(expanseMapList[index]),).toList();
  }

  @override
  Future<void> updateExpanse(Expanse expanse) async {
    Database? db = await dbCreate.database;
    await db!.update(expanseTableName,expanse.toJson(),where: '$expanseColumnId = ?',whereArgs: [expanse.id]);
